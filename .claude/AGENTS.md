...
- **Signature de code** : releases **non signées** pour l'instant — candidature SignPath Foundation **refusée** (2026-08, projet jugé trop peu connu) ; assumé publiquement dans le README (section « A note on SmartScreen »). Le plan `.claude/plan-signpath.md` est gelé avec les pistes de relance (re-candidater avec plus de traction, ou certificat OV payant). Winget reste un canal valable sans signature. L'EV ne bypass plus SmartScreen (2024) : la réputation se construit avec les téléchargements et se transfère entre versions d'un même certificat.
